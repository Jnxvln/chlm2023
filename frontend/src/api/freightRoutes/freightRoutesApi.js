...
// Get freight routes by vendor ID
export const getFreightRoutesByVendorId = async (vendorId, token) => {
    const config = {
        headers: {
            Authorization: `Bearer ${token}`,
        },
    }
    const response = await axios.get(API_URL + `/vendor/${vendorId}`, config)
    return response.data
}
