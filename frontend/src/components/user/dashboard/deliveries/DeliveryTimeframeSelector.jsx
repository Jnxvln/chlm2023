...
function DeliveryTimeframeSelector({ onDateRangeSelected }) {
  ...
  if (_selectedDeliveriesDateRange && _selectedDeliveriesDateRange.length > 0) {
    for (let i = 0; i < _selectedDeliveriesDateRange.length; i++) {
      _selectedDeliveriesDateRange[i] = new Date(_selectedDeliveriesDateRange[i]);
    }
  }
  ...
  const onChange = (e) => {
    ...
    localStorage.setItem("selectedDeliveriesDateRange", JSON.stringify(_dateRangeSelected));
